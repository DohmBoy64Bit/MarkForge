...
function startPreview(port) {
  ...
  const viteCli = path.join(editorRoot, 'node_modules', 'vite', 'bin', 'vite.js')
  const child = spawn(
    process.execPath,
  ...
  child.stdout.on('data', chunk => process.stdout.write(`[editor-preview] ${chunk}`))
  child.stderr.on('data', chunk => process.stderr.write(`[editor-preview] ${chunk}`))
  return child
}
...
async function assertVisibleControls(page) {
  for (const name of toolbarControls) {
    await visible(page.getByRole('button', { name, exact: true }), `toolbar control ${name}`)
  }

  for (const control of formattingControls) {
    await visible(page.getByRole('button', { name: control }).first(), `formatting control ${control}`)
  }

  for (const label of ['Search source', 'Replace with']) {
    await visible(page.getByLabel(label), `${label} input`)
  }
  ...
  for (const heading of ['Workspace', 'File Status', 'Commands', 'Converters', 'Export Profile', 'Search', 'Outline', 'Recent Files', 'Front Matter', 'Warnings']) {
    await visible(page.getByRole('heading', { name: heading }), `inspector section ${heading}`)
  }
}

async function assertEveryVisibleInteractiveElementHasName(page, context) {
  const unlabeled = await page.evaluate(() => {
    const controls = Array.from(document.querySelectorAll('button, input, select, textarea, [role="button"], [role="option"], [contenteditable="true"]'))
...
async function assertNoHorizontalOverflow(page, context) {
  ...
  assert(metrics.mainHeight > 400, `${context} did not render a full editor shell: ${JSON.stringify(metrics)}`)
  assert(metrics.visibleButtons > 25, `${context} rendered too few visible buttons: ${JSON.stringify(metrics)}`)
}
...
async function exerciseCoreEditor(page) {
  await assertVisibleControls(page)
  await assertEveryVisibleInteractiveElementHasName(page, 'desktop editor shell')
  await assertNoHorizontalOverflow(page, 'desktop editor shell')
  ...
  summary.clickedWorkflows.push('theme switcher')

  for (const mode of ['source', 'rich', 'split', 'preview']) {
    await page.getByRole('button', { name: new RegExp(`${mode} view`, 'i') }).click()
    await assertNoHorizontalOverflow(page, `${mode} view`)
  }
  await page.getByRole('button', { name: /split view/i }).click()
  summary.clickedWorkflows.push('view mode switcher')
  ...
  summary.clickedWorkflows.push('search and replace controls')

  await page.getByRole('button', { name: /Format Markdown/ }).click()
  await page.getByRole('button', { name: /Align selected Markdown table/ }).click()
  await page.getByRole('button', { name: /Insert table column after cursor/ }).click()
  summary.clickedWorkflows.push('formatting and table controls')
}

async function exerciseDialogs(page) {
  ...
  summary.clickedWorkflows.push('command palette')
  ...
  summary.clickedWorkflows.push('quick insert table')
  ...
  await screenshot(page, 'editor-import-conversion.png')
  await page.getByRole('button', { name: 'Insert', exact: true }).click()
  await visible(page.getByText('Converted HTML').first(), 'converted HTML output')
  summary.clickedWorkflows.push('HTML import conversion')
  ...
  summary.clickedWorkflows.push('templates and help custom template/reference')
  ...
  await screenshot(page, 'editor-preferences.png')
  await page.getByRole('button', { name: 'Close preferences' }).click()
  summary.clickedWorkflows.push('preferences')
  ...
  await visible(page.getByText(/Document text is sent only to the configured loopback provider/i), 'Local AI privacy copy')
  await screenshot(page, 'editor-local-ai.png')
  await page.getByRole('button', { name: 'Close Local AI' }).click()
  summary.clickedWorkflows.push('Local AI profile workflow')
}

async function exerciseMobile(page) {
  await page.setViewportSize({ width: 390, height: 844 })
  await assertNoHorizontalOverflow(page, 'mobile editor shell')
  await assertEveryVisibleInteractiveElementHasName(page, 'mobile editor shell')
...
async function run() {
  ...
  try {
    ...
    await exerciseCoreEditor(page)
    await exerciseDialogs(page)
    await exerciseMobile(page)
