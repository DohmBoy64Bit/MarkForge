...
| Commands | Command palette | docs basics; commandPalette component | partially supported | `apps/editor`, `packages/editor-engine` | Phase 5C adds a MarkForge-native command palette over the editor-engine command registry with search, grouping, keyboard navigation, shortcut display, and command execution; quick open and non-format commands remain future scope. |
...
| Keybindings | Per-platform default keybindings | `keybindingsWindows.ts`, `keybindingsLinux.ts`, `keybindingsDarwin.ts` | partially supported | `apps/editor`, `packages/platform`, `packages/ui` | MarkForge supports Windows/Linux first; Phase 5C adds Ctrl+Shift+P for the command palette plus the existing source formatting shortcuts; macOS-specific defaults are not required yet. |
