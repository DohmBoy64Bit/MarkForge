...
| --- | --- | --- | --- | --- | --- | --- |
| DW-01 | Deferred | Completed by Phase 11 for opened files and editor close protection; follow-up adds editor/viewer workspace services | Implemented `packages/platform` with filesystem, dialogs, clipboard, print, watch, workspace list/search/watch, shell recent, spellcheck, updater status, and close-protection contracts; Phase 11 adds Rust `notify` open-file watchers, workspace watchers, and Tauri close-request interception. | Report DW-01; architecture platform rules; editor/viewer direct Tauri calls. | `packages/platform/src/index.spec.ts`; builds; Rust checks. | Broader workspace/network smoke remains unsupported. |
...
| DW-07 | Deferred | Partially completed | Built-in, local custom, and workspace templates are implemented, and line-leading template plus Markdown structure suggestions are package-owned. | Templates/editor-engine docs/source; app custom-template storage and workspace loading. | Existing template/custom-template/autocomplete tests. | Syncable template libraries and path-aware autocomplete still needed. |
...
| AD-03 | Partially fixed | Partially completed | Added real dependency edges and docs-check coverage; editor preview/template APIs now flow through `packages/editor-engine`. | App package manifests/source. | Builds/docs-check. | App shells still own React orchestration and Tauri adapter wiring. |
...
| TD-01 | Deferred | Partially completed | Platform owns polling watch fallback and metadata calls; Phase 11 adds native opened-file watch commands/events for editor and viewer, and follow-up remediation adds editor/viewer workspace watch commands/events. | App/platform source; Phase 11 docs. | Platform tests, builds, Phase 11 Rust checks. | Broader workspace/network smoke remains unsupported. |
...
Unsupported capabilities are explicit package results or documented remaining work: native spellcheck providers, auto-update publishing/signing, Linux package artifacts, native PDF/DOCX/OCR conversion, local AI streaming and persisted provider profiles, WYSIWYG/realtime editing beyond CodeMirror source mode, path-aware/link-aware autocomplete, syncable template libraries, and broader workspace/network smoke evidence. Broader route-level lazy loading remains optional future optimization rather than current chunk-warning debt.
