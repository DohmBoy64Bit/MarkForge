...
- Editing Markdown.
- Source, rich, preview, and split modes.
...
- Convert to Markdown from HTML, CSV/table text, rich clipboard HTML, URL/article HTML, DOCX, PDF text, and OCR image input.
...
The editor toolbar includes **Local AI**. The dialog starts disabled, requires an explicit local provider enable toggle, accepts only loopback provider endpoints, persists local provider profiles, streams result text when the provider supports it, and inserts returned text only after the user chooses an insert action.
