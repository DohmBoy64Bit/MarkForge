...
- Phase 7C adds editor Import Conversion for supported HTML-to-Markdown and CSV-to-Markdown-table conversions. Converted Markdown can replace the current selection, insert at the cursor, or append to the document through tested insertion helpers.
...
Phase 7F exposes only the Phase 7A capabilities that are safe in the current apps: Markdown-to-HTML export, HTML-to-Markdown import, CSV-to-Markdown table import, Markdown cleanup in the editor, and browser-print handoff. DOCX, native PDF, OCR, URL/article extraction, and rich clipboard conversion are not surfaced as working UI actions.
