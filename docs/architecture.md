...
- `packages/converters` owns the converter contract, sanitized HTML export with basic export settings, HTML-to-Markdown import, CSV-to-Markdown table conversion, rich clipboard HTML import, URL/article import, DOCX import/export, PDF text import, Markdown-to-PDF export, OCR image import, Markdown cleanup, capability checks, browser-print pathway, warnings, and unsupported capability results.
- `packages/llm` owns local-only provider contracts, prompt templates, local action execution, streaming action execution, provider profile helpers, mock provider tests, cancellation, loopback endpoint validation, Ollama and OpenAI-compatible local adapters, explicit unsupported local adapter boundaries, and the privacy guard. The editor now exposes a disabled-by-default Local AI workflow through this package.
...
- `apps/editor` now delegates preference/session/recent-file schema behavior to `packages/core`, platform text/binary read/write/dialog/clipboard/print/watch/workspace/shell-recent/close behavior to `packages/platform`, supported conversion execution to `packages/converters`, app theme variables to `packages/theme-engine`, editor preview/template/autocomplete/search/insertion APIs to `packages/editor-engine`, and Local AI provider execution to `packages/llm`. The source editor surface is CodeMirror-backed and rich Markdown mode is ProseMirror-backed inside the app shell while command transforms remain package-owned in `packages/editor-engine`. The app still owns document orchestration, source search UI state, workspace panel state, converter workflow UI/activity history, Local AI dialog state, command UI wiring, custom template UI persistence, and live preview composition.
...
- Signing keys, updater publishing endpoints, Linux packaging artifacts, richer native spellcheck providers, full document-conversion fixture coverage, and rich-editing parity hardening remain release/runtime or larger architecture work because they require external keys, release hosts, native Linux prerequisites, provider decisions, or broader fixture suites. Workspace/folder watching, workspace search, workspace templates, rich clipboard HTML import, URL import, DOCX/PDF/OCR conversion, general Markdown slash autocomplete, path-aware link/image autocomplete, source formatting/image/table/table-row/delete-line commands, rich Markdown editing, and Windows shell recent-document updates now have concrete package/app implementations.
